...
var clicks = []
var points = [] //定义粒子数组
var live_point = 15 //存活10个周期
var live_click = 60 //存活60个周期
var colors = [  //备选粒子颜色数组
    "236, 204, 104",
    "255, 71, 87",
    "112, 161, 255",
...
function drawpoints() { //绘制粒子
    ...
    for (var i = 0; i < points.length; i++) { //遍历粒子
        var point = points[i] //定义单个粒子
        ctx.beginPath()
        ctx.arc(point.sx, point.sy, point.size, Math.PI * 2, false) //根据粒子属性画圆
        ctx.fillStyle = "rgba(" + point.color + "," + point.life / live_point + ")" //根据粒子属性设置填充颜色及透明度
        ctx.fill() //填充颜色
        point.life-- //生命值减1
    ...
    for (var i = 0; i < clicks.length; i++) { //绘制点击效果
        var click = clicks[i]
        ctx.fillStyle = "rgba(" + click.color + "," + click.life / live_click + ")"
